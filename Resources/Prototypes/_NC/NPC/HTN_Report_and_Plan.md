...
### Что мы сделали с NPC
- Разделили NC NPC на три категории: бандиты, Biotechnica и Militech.
- Создали разные HTN root-задачи:
  - `NCBanditRootCompound` для базового уровня.
  - `NCBanditTier2RootCompound` для среднего уровня.
  - `NCBanditTier3RootCompound` для сильного уровня.
- Добавили разное поведение через `IdleRange`:
  - бандиты — меньше гуляют,
  - Biotechnica — больше,
  - Militech — ещё дальше.
- Присвоили NPC новые фракции:
  - `NCBandit`, `NCBiotechnica`, `NCMilitech`.
- Сделали эти фракции враждебными друг к другу.
- Организовали NPC в структуры `Lootable` / `Unlootable` и обновили спавнеры.

### Что ещё не добавлено
- Реальный поиск укрытий и использование cover.
- Фланговые манёвры или обход цели.
- Координация между NPC: поддержка союзников, общий выбор цели.
- Логика отступления при низком HP.
- Разделение боевого поведения внутри фракций по классу оружия.
- Подбор патронов и сложные операции с боезапасом.

### Что можно улучшить
- Для бандитов: усилить патрулирование и мобильность в idle, но оставить простое поведение.
- Для Biotechnica: добавить укрытия, умный выбор целей и паузы после атаки.
- Для Militech: повысить приоритет стрельбы и агрессивное движение.
- Общие улучшения: разделить HTN на специализированные ветки `RetreatCompound`, `SupportCompound`.
- Использовать существующие утилиты (`GunAmmoPrecondition`) для контроля состояния оружия, но без попыток подбора боезапаса.
